import React from 'react'

const Navbar = () => {
  return (
    <div className='flex justify-between items-center px-10 p-4 text-sm font-medium'>
      <div >
        <img className="h-3"src="data:image/svg+xml;base64," alt="" />
      </div>

      <div className='hidden lg:inline'>
        <ul className='flex justify-center hover:cursor-pointer'>
          <li className='py-1 px-3 hover:shadow'>Vehicles</li>
          <li className='py-1 px-3 hover:shadow'>Energy</li>
          <li className='py-1 px-3 hover:shadow'>Charging</li>
          <li className='py-1 px-3 hover:shadow'>Discover</li>
          <li className='py-1 px-3 hover:shadow'>Shop</li>
        </ul>
      </div>
      <div className='hidden lg:inline'>
        <ul className='flex justify-center hover:cursor-pointer'>
          <li className='py-1 px-3'>Support</li>
          <li className='py-1 px-3'>Location</li>
          <li className='py-1 px-3'>Account</li>
        </ul>
      </div>
      <div className='lg:hidden'>
        <button className='inline-flex items-center rounded-md py-2 px-4 text-sm font-medium bg-black/5 shadow-sm hover:bg-black/10 m-0'>Menu</button>
      </div>
    </div>
  )
}

export default Navbar